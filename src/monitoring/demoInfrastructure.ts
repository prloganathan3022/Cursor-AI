import type { InfrastructureHealth, MonitoringAlert, MonitoringLogEntry } from './types'
...
export function tickInfrastructure(
  ...
  const cpu = Math.min(
    ...
    Math.max(
      5,
      prev.cpuPercent + (rnd(t) - 0.48) * 6 + Math.sin(t / 12) * 2,
    ),
  )
  ...
  const heap = Math.min(512, Math.max(48, prev.heapUsedMb + (rnd(t + 3) - 0.45) * 8))
  const rps = Math.max(
    0,
